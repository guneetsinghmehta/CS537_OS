...
mkfs(int nblocks, int ninodes, int size) {
  ...
  usedblocks = ninodes / IPB + 3 + bitblocks;// zeroth block,superblock, first inode block =3 . ninodes/IPB- any additional blocks for inodes
  freeblock = usedblocks;//first free block
...
winode(uint inum, struct dinode *ip)
{
  ...
  bn = i2b(inum);
  rsect(bn, buf);
  dip = ((struct dinode*)buf) + (inum % IPB);//points to a address x
  *dip = *ip;//puts value at address x as value in ip
  wsect(bn, buf);
}
