...
int priority_checker()
{
  ...
  struct proc *p;
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
        if(p->pri==2 && p->state==RUNNABLE){
            //release(&ptable.lock);
            return 1; //process with pri ==2
        }
    }
    //release(&ptable.lock);
...
void scheduler(void)
{
  ...
  for(;;){ 
    ...
    //acquires lock
    acquire(&ptable.lock);
    //round robins through processes. ptable.proc is the first entity`
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state!=RUNNABLE) continue;
          ...
          {     
              if(priority_checker()) //p with pri==2 exists, so retry in the inner for loop
                  continue;
          }
...
kill(int pid)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->pid == pid){
      //p-pri = 1; //resetting p's priority //amohanty
      p->killed = 1;
