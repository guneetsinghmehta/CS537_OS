...
int shmem_proc_count[SHMEM_PAGES];
...
freevm(pde_t *pgdir)
{
  ...
  //cprintf("shared pages in  freevm%d",proc->procShmemCount);
...
int shmem_count(int page_number)
{
	...
	return shmem_proc_count[page_number];
    else
    	panic("invalid shmem access");
}

void* shmem_access(int page_number)
{
    //checking invalid page number request
    if(page_number<0||page_number>3)
    {return NULL;}

    //get physical address of the shared page from structure in vm.c
	...
	{	
		//cprintf("proc bitmap = %d\n",proc->bitmap[page_number]);//-1 if invlaid and non negative if valid
		if(proc->bitmap[page_number]==-1)
		{
			...
			//shmem_proc_count[page_number]++;//not working
			proc->bitmap[page_number]=USERTOP-4096*i;//updating bitmap of process, assigning the VA mapped to the page
			//cprintf("proc bitmap = %d\n",proc->bitmap[page_number]);//print check
		}
		return (void*)(proc->bitmap[page_number]);
	}
	return NULL;
}
//used to overcome problem of accessing 	shmem_proc_count[index]  through headers 
void inc_shmem_proc_count(int index)
{
	shmem_proc_count[index]++;
}

void dec_shmem_proc_count(int index)
{
	shmem_proc_count[index]--;
}
