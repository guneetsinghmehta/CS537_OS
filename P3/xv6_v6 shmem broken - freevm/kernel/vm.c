...
freevm(pde_t *pgdir)
{
  ...
  int a=0;
  if(proc->procshmem_count>0){a=1;}
  for(i = 0; i < NPDENTRIES-a; i++){
    if(pgdir[i] & PTE_P)
...
//    pte_t *lastAvailPage=
	...
	for(i=1;i<=4;i++)
	{
		...
		proc->procshmem_count=i-1;//say i=2 then the process has 1 shared page allocated, if 5 then 4 pages are shared
